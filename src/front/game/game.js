        ...
        if (cursor.i < settings.sizes.f_height && cursor.j < settings.sizes.f_width) {
            if (settings.current.enemy_field[cursor.i][cursor.j] === 1 ||
                ...
                if (is_killed(cursor.j, cursor.i, settings.current.enemy_field, 0)) {
                    ...
                    const myurl = window.location.origin + "/api/users/" + current_id;
            ...
            if (check_end_of_game() === 1) {
                ...
                const res = await fetch(myurl);
                var json = await res.json();
                let won = json.games * json.winrate
                json.games++;
                json.winrate = (won + 1) / json.games
                senddata(json);
            ...
            switch (settings.current.field[y][x]) {
                ...
                case (1):
                    ...
                    if (check_end_of_game() === 2) {
                        ...
                        let won = myjson.games * myjson.winrate
                        myjson.games++;
                        myjson.winrate = won / myjson.games
                        senddata(myjson);
...
async function senddata(data) {
    const url = window.location.origin + "/api/users/" + current_id;
    console.log(url);
    console.log(data);
    try {
